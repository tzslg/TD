...
from dumbotd_2.core.nwabc import nwatomicbroadcast
...
class Dumbo_TD_k_s:
    def __init__(self, sid, pid, S, B, N, f, sPK, sSK, sPK2s, sSK2, send, recv, K=3, countpoint=0,mute=False,debug=False):
        ...
        self.epoch = 0
        self.record_point = [0 for _ in range(N * self.K)]
                    ...
                    for i in range(N):
                        for j in range(K):
                            ...
                            while True:
                                ...
                                if epoch == 0:
                                    (r, stripe_r, hash, b_, sid) = self.fixed_Tx[i * self.K + j].get()
                                    rc_msg[i * self.K + j].add(('RCSTORE', r, stripe_r, hash, self.id))
                        ...
                        for j in msg_r[0]:
                            ...
                            if len(store_r) == f + 1:
                                ...
                                for k in range(N - f - 1):
                                    share.append(None)
                                TX = decode(f + 1, N, share)



                                stripes = encode(f+1, N, str(TX).encode('utf-8'))
