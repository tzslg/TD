...
from dumbotd.core.nwabc import nwatomicbroadcast
...
class Dumbo_TD_k_s:
    def __init__(self, sid, pid, S, B, N, f, sPK, sSK, sPK2s, sSK2, send, recv, K=3, countpoint=0,mute=False,debug=False):
        ...
        self.fixed_Tx = [multiprocessing.Queue() for _ in range(N * self.K)]
    ...
    def run_bft(self):
        ...
        def _finalize_output():
            ...
            def _run_VABA_round(tx_to_send, send, recv):
                ...
                out = vaba_output.get()
                (view, sig, txhash) = out
                self.record_point.put_nowait(view)
                self.record_point_out.put_nowait(view)
                        ...
                        for j in range(self.K):
                            if self.output_list[i * self.K + j].qsize() > 1:
                                ...
                                if s > cur_view[i * self.K + j]:
                                    ...
                                    if self.epoch > 20:
                                        del_p = max(0, cur_view[i * self.K + j] - 20)
                                        try:
        ...
        def recastsubprotocol():
            ...
            def Share_distribution():
                rc_epoch = 0
                current_record = [0 for _ in range(N * K)]
                rc_send = _make_send_rc()
                while True:
                    previous_record = current_record
                    while True:
                        try:
                            current_record = self.record_point.get_nowait()
                            break
                        except:
                            gevent.sleep(0.1)
                    rc_msg = [set() for _ in range(N * K)]
                    for i in range(N):
                        for j in range(K):
                            try:
                                if rc_epoch == 0:
                                    for _ in range(current_record[i * self.K + j] + 1):
                                        while True:
                                            try:
                                                (r, stripe_r, _, _, _) = self.fixed_Tx[i * self.K + j].get_nowait()
                                                break
                                            except:
                                                gevent.sleep(0.1)
                                        rc_msg[i * self.K + j].add(('RCSTORE', r, stripe_r))
                                else:
                                    if current_record[i * self.K + j] - previous_record[i * self.K + j] > 0:
                                        for _ in range(current_record[i * self.K + j] - previous_record[i * self.K + j]):
                                            while True:
                                                try:
                                                    (r, stripe_r, _, _, _) = self.fixed_Tx[i * self.K + j].get_nowait()
                                                    break
            ...
            def output_r():
                nonlocal TX,block
                epoch_r = 0
                store_r = defaultdict(lambda: defaultdict(list))
                def output_block():
                    nonlocal epoch_r
                    current_record_r = [0 for _ in range(N)]
                    while True:
                        previous_record_r = current_record_r
                        while True:
                            try:
                                current_record_r = self.record_point_out.get_nowait()
                                break
                            except:
                                gevent.sleep(0.1)
                        k = 0
                        if epoch_r == 0:
                            for m in current_record_r:
                                for n in range(m+1):
                                    while True:
                                        try:
                                            if TX[i][n]:
                                                block[epoch_r].add(TX[i][n])
                                                break
                                        except:
                                            gevent.sleep(0.1)
                        else:
                            for m in current_record_r:
                                for n in range(previous_record_r[k], m):
                                    while True:
                                        try:
                                            if TX[i][n]:
                                                block[epoch_r].add(TX[i][n])
                ...
                def share_aggregation():
                    nonlocal TX, epoch_r, store_r
                    while True:
                        while True:
                            try:
                                recv = self.rc_recv.get_nowait()
                                _, msg_r = recv
                                break
                            except:
                                gevent.sleep(0.1)
                        for i in range(N * self.K):
                            for j in msg_r[i]:
                                store_r[i][j[1]].append(j[2])
                                if len(store_r[i][j[1]]) == f + 1:
                                    for k in range(N - f - 1):
                                        store_r[i][j[1]].append(None)
                                    TX[i][j[1]] = decode(f + 1, N, store_r[i][j[1]])
                                    del store_r[i][j[1]] 
                                    
                        gevent.sleep(0.1)
                sa = gevent.spawn(share_aggregation)
                ob = gevent.spawn(output_block())
                gevent.joinall([sa,ob])
            sd = gevent.spawn(Share_distribution)
            otr = gevent.spawn(output_r)
            gevent.joinall([sd, otr])
        self._abcs = Process(target=abcs)
        self._abcs.start()
        self._rc = Process(target=recastsubprotocol)
        self._rc.start()
        self._recv_output = gevent.spawn(_finalize_output)
        #self._rc = gevent.spawn(recastsubprotocol)
    ...
    def _run_nwabc(self, send, recv, i, j):
        ...
        t = gevent.spawn(nwatomicbroadcast, epoch_id + str(i * self.K + j), pid, N, f, self.B, self.sPK2s, self.sSK2, leader,
                         self.transaction_buffer[j].popleft, self.output_list[i * self.K + j].put_nowait, self.fixed_Tx[i * self.K + j].put_nowait, recv, send, self.logger)
